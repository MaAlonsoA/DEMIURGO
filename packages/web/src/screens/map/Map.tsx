...
// the design system's FeatureCard and the rules they follow as its Node, the relations their links
// declare drawn between them, the questions waiting on the person and the parked ideas. Pointing
// lights up an element's connections; a click (or Enter) keeps it in the panel; −, 100%, + and Fit
// zoom.
...
import { RECORD_ICON, RECORD_TYPE } from '../../ui/icons.tsx';
      ...
      {map.isPending ? (
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-4">
            <p className="dm-text-small text-ink-2">
              ...
              {data.questions.length > 0 && (
                <span className="font-semibold text-needs-strong">
                  {' '}
            ...
            <div role="group" aria-label="Zoom" className="flex items-center gap-1">
              <Button variant="secondary" aria-label="Zoom out" onClick={() => step(-1)}>
                −
              </Button>
              <Button variant="text" aria-label="Actual size" onClick={() => setScale(1)} className="min-w-16 tabular-nums">
                {Math.round(scale * 100)}%
              </Button>
              <Button variant="secondary" aria-label="Zoom in" onClick={() => step(1)}>
                +
              </Button>
              <Button variant="text" onClick={fit}>
                Fit
              </Button>
            </div>
          </div>
          <div
            ref={viewport}
            data-map
            className="relative h-[calc(100vh-230px)] min-h-[420px] overflow-auto rounded-panel border border-line bg-paper bg-[radial-gradient(var(--line)_1px,transparent_1px)] [background-size:20px_20px]"
          >
            <div
              ref={content}
            ...
            >
              ...
              {lanesOf(data).map((lane) => (
                <section
                  ...
                  data-lane={lane.area}
                  className="relative z-10 flex w-[300px] flex-col gap-3"
                >
                  <h2 className="dm-label border-b border-line pb-1.5">{lane.area}</h2>
                  ...
                  {lane.rules.length > 0 && (
                    <h3 className="dm-text-caption mt-2 font-semibold text-muted">
              ...
              {data.ideas.length > 0 && (
                <section aria-label="Parked ideas" className="relative z-10 flex w-[260px] flex-col gap-3">
                  <h2 className="dm-label border-b border-dashed border-inactive pb-1.5">Parked ideas</h2>
                  {data.ideas.map((i) => (
                    <Link
                      key={i.id}
                      to="/p/$projectId/threads/$explorationId"
                      params={{ projectId, explorationId: i.id }}
                      className="block rounded-card-md"
                    >
                      <DsNode type="idea" state="parked" title={i.purpose} />
                    </Link>
                  ))}
                </section>
              )}
...
function MapElement({
  ...
  const detail = `${waiting} ${waiting === 1 ? 'question waits' : 'questions wait'} on you`;
  return (
    <button
      type="button"
      ...
      className={cn(
        'block w-full rounded-card text-left transition-opacity focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-needs',
        dimmed && 'opacity-40',
      )}
    >
      {row.type === 'fdr' ? (
        <DsFeatureCard
          ...
          when={ago(row.updated_at)}
          signals={row.checks > 0 ? <Signal kind="checks" value={row.checks} /> : undefined}
          selected={selected}
        />
      ) : (
        <DsNode
          ...
          title={row.title}
          trailing={<NeedsBubble count={waiting} detail={detail} />}
          selected={selected}
        />
      )}
    </button>
...
function LegendLine({ kind, label }: { kind: MapRelation['kind']; label: string }) {
  ...
  return (
    <li className="dm-text-small flex items-center gap-2 text-ink-2">
...
function MapLegend() {
  return (
    <section aria-labelledby="map-legend" className="flex flex-col gap-3">
      <h2 id="map-legend" className="dm-text-heading">
        How to read the map
      </h2>
      ...
      <p className="dm-text-caption text-muted">Only the links the records declare are drawn: nothing is guessed.</p>
    </section>
  );
}
      ...
      {row.origin_exploration && (
        <PanelBlock title="Comes from">
          <Link
            ...
            className="dm-text-small font-medium text-ink underline-offset-2 hover:underline"
          >
            Its thread
          </Link>
        </PanelBlock>
            ...
            {codes.map((c) => (
              <li key={c}>
                <Link
                  ...
                  className="dm-text-small flex items-baseline gap-2 text-ink underline-offset-2 hover:underline"
                >
                  {title(c)} <Code>{c}</Code>
                </Link>
              </li>
      ...
      {waiting.length > 0 && (
        <PanelBlock title="Waiting on you" tone="needs">
          <ul className="flex flex-col gap-2">
            {waiting.map((q) => (
              <li key={q.id} className="dm-text-small flex items-start justify-between gap-3">
...
function PanelBlock({ title, tone, children }: { title: string; tone?: 'needs'; children: ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5 border-t border-line-soft pt-3">
      <h3 className={cn('dm-text-caption font-semibold', tone === 'needs' ? 'text-needs-strong' : 'text-muted')}>{title}</h3>
      {children}
    </div>
  );
}
