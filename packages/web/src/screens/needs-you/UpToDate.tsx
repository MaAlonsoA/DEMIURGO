...
// Catch up). What they did today, told with the lines of the lens; where the product is; what
// DEMIURGO is still doing, in amber; and that they can close DEMIURGO: everything is saved.
...
export function UpToDate({ projectId }: { projectId: string }) {
  ...
  return (
    <Page
      ...
      asideFooter={
        <div data-close className="flex flex-col gap-1.5 rounded-xl bg-surface-2 px-4 py-3.5">
          <strong className="text-[13px] font-semibold">{PRODUCT_WORDS.nothingNeedsYou}</strong>
    ...
    >
      ...
      <section
        aria-labelledby={id}
        className="flex max-w-[1000px] flex-col gap-1 rounded-[var(--radius-panel)] border border-line bg-surface px-5 pt-3.5 pb-3"
      >
        <h2 id={id} className="mb-1 text-[15px] font-semibold">
          Today
        ...
        {lines === null ? (
          ...
          <p className="py-1 text-[13.5px] text-ink-3">Nothing yet today. What you do here is kept and shown next time.</p>
        ) : (
          <ul className="flex flex-col">
            {lines.map((l) => (
              <li
                key={l.id}
                data-today-line={l.id}
                className="-mx-2 grid grid-cols-[64px_18px_minmax(0,1fr)_auto] items-center gap-2.5 rounded-lg px-2 py-[5px] text-[13.5px] hover:bg-surface-2"
              >
                <span className="text-xs whitespace-nowrap text-muted" title={dayTime(l.at)}>
                  {time(l.at)}
                </span>
      ...
      {state && (
        <section aria-labelledby={productId} className="mt-7 flex max-w-[1000px] flex-col gap-1">
          <div className="flex items-baseline justify-between gap-4">
            <h2 id={productId} className="text-xs font-semibold text-muted">
              The product · {state.project.name}
            </h2>
            <Link to="/p/$projectId" params={{ projectId }} className="text-xs font-semibold text-needs hover:text-needs-hover">
              See the product
            </Link>
          </div>
