    ...
    <Panel className="flex max-w-[860px] flex-col gap-5">
      <div className="flex items-start justify-between gap-6">
        <div className="flex flex-col gap-1">
          <h3 className="dm-text-heading font-semibold">Rebuild from what you approved</h3>
        ...
        <Button variant="secondary" onClick={() => void rebuild.refetch()} disabled={rebuild.isFetching} className="shrink-0">
      ...
      {rebuild.error ? (
        ...
        <>
          <dl className="grid grid-cols-[220px_minmax(0,1fr)] gap-x-4 gap-y-3 border-y border-line-soft py-4">
            <dt className="dm-text-small text-ink-2">
              Live graph
              {version !== undefined && <span className="text-muted"> · version {version}</span>}
            </dt>
            <dd data-fingerprint="live" className="dm-text-caption font-mono break-all text-ink">
              {r.live}
            </dd>
            <dt className="dm-text-small text-ink-2">Rebuilt from what you approved</dt>
            <dd data-fingerprint="rebuilt" className="dm-text-caption font-mono break-all text-ink">
              {r.rebuilt ?? <span className="font-sans text-muted">It could not be rebuilt.</span>}
            </dd>
          </dl>
          {r.equal ? (
            <div className="flex flex-col gap-1">
              <MarkWord kind="confirmed" word="They match" className="dm-text-body" />
              <p className="dm-text-small text-ink-3">
                The graph is exactly what your records and its saved classifications give.
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-1.5 rounded-control bg-problem-tint px-3 py-2.5">
              <p className="dm-text-body flex items-center gap-1.5 font-semibold text-problem">
                <WarningIcon size={14} />
                They don’t match
              </p>
              {r.drift && <p className="dm-text-small text-problem">{r.drift}</p>}
            </div>
          )}
        </>
      )}
