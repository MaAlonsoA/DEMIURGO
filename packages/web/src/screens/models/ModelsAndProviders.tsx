...
function ModelsAndProviders({ projectId }: { projectId?: string }) {
  ...
  return (
    <div className="flex max-w-[1240px] flex-col gap-9">
      <PageTitle
        eyebrow="Settings"
...
function StatusDot({ tone }: { tone: 'ok' | 'problem' | 'off' }) {
  return (
    <span
      aria-hidden="true"
      className={cn(
        'dm-dot dm-dot--sm shrink-0',
        tone === 'ok' && 'bg-ink',
        tone === 'problem' && 'bg-problem-fill',
        tone === 'off' && 'bg-inactive-soft',
      )}
    />
  );
}
...
function ProviderCard({ catalog: c }: { catalog: Catalog }) {
  ...
  return (
    <article data-provider={c.provider} className="dm-card gap-3 px-4 py-3.5">
      <header className="flex items-baseline justify-between gap-3">
        <h3 className="dm-text-heading">{c.label}</h3>
        <span className="dm-text-caption flex items-center gap-1.5 text-ink-2">
          <StatusDot tone={tone} />
          {state}
          {c.version && <Code>v{c.version}</Code>}
        </span>
      </header>
      {c.message && <p className={cn('dm-text-small', c.ready ? 'text-ink-2' : 'text-problem')}>{c.message}</p>}
      {c.models.length > 0 ? (
        <ul className="flex flex-col gap-1.5">
          {c.models.map((m) => (
            <li key={m.id} className="dm-text-small flex flex-wrap items-baseline gap-x-2 gap-y-1">
                ...
                <span className="flex flex-wrap gap-1">
                  {m.efforts.map((e) => (
                    <span
                      key={e}
                      className={cn(
                        'dm-text-caption rounded-tag border px-1.5',
        ...
        <p className="dm-text-small text-muted">No models to offer.</p>
      )}
      <footer className="dm-card-foot flex-wrap justify-start gap-x-1.5 gap-y-0.5">
...
const agentColumns = (projectId?: string) =>
  projectId
    ? 'grid-cols-[minmax(220px,1fr)_minmax(480px,2fr)_minmax(480px,2fr)]'
    : 'grid-cols-[minmax(220px,1fr)_minmax(480px,2fr)]';
...
function AgentTable({ agents, catalogs, projectId }: { agents: AgentInfo[]; catalogs: Catalog[]; projectId?: string }) {
  ...
  return (
    <div className="overflow-hidden rounded-card border border-line bg-surface">
      <div
        className={cn(
          'dm-text-caption grid border-b border-line-soft bg-surface-soft px-4 py-2 font-semibold text-muted',
  ...
  return (
    ...
    >
      <div className="flex min-w-0 flex-col gap-0.5 pr-4">
        <span className="dm-text-body font-semibold text-ink">{a.section}</span>
        <span className="dm-text-small text-ink-2">{a.description}</span>
        ...
        <span
          data-effective
          className={cn(
            'dm-text-caption mt-1 flex items-center gap-1.5',
      ...
      <Cell>
        ...
        />
        {a.global ? (
          <Button variant="text" disabled={busy} onClick={() => unassign.mutate('global')}>
            Remove
          </Button>
        ) : (
          <span className="dm-text-caption font-semibold text-problem">No model yet</span>
        )}
      </Cell>
      {projectId && (
        <Cell>
          {a.project || overriding ? (
            ...
            <>
              <span className="dm-text-small text-muted">Same as everywhere</span>
              <Button
                variant="secondary"
...
function UsageSection({ consumption, catalogs }: { consumption: Consumption; catalogs: Catalog[] }) {
  ...
  return (
    <section aria-labelledby="usage-title">
        ...
        aside={
          <span className="flex gap-1" role="group" aria-label="Period">
            {(['today', 'week'] as const).map((p) => (
              <Chip key={p} pressed={period === p} onClick={() => setPeriod(p)}>
      ...
      <p className="dm-text-small mb-3 text-muted">
...
function UsageTable({ title, rows, name }: { title: string; rows: ConsumptionRow[]; name: (key: string) => string }) {
  return (
    <div className="rounded-card border border-line bg-surface">
      <h3 className="dm-text-small border-b border-line-soft px-4 py-2 font-semibold">{title}</h3>
      {rows.length === 0 ? (
        <p className="dm-text-small px-4 py-3 text-muted">Nothing yet.</p>
      ) : (
        <table className="dm-text-small w-full">
          <thead>
            <tr className="dm-text-caption text-left text-muted">
  ...
  return (
    <section aria-labelledby="stats-title">
      ...
      <div className="overflow-hidden rounded-card border border-line bg-surface">
        <table className="dm-text-small w-full">
          <thead>
            <tr className="dm-text-caption border-b border-line-soft bg-surface-soft text-left text-muted">
          ...
          <tbody>
            {stats.map((s) => (
              ...
              >
                ...
                <td className="dm-text-caption px-2 py-1.5 text-problem">
