...
Los vocabularios cerrados están en `packages/domain/src/classifier.ts` (`VERDICTS` e `IDEA_FINDINGS`). El formato y las métricas están en `packages/domain/src/metrics.ts`.

## Ficheros

| Fichero | Casos | Qué contiene |
|---|---|---|
| `verdicts.jsonl` | 69 | Pares (cambio, candidato) con el veredicto esperado |
...
```json
{"id":"V002","partition":"test",
 "change":{"ref":"DEC-CUO-008@2","type":"decision","title":"Importe de la cuota anual","text":"La cuota anual pasa a ser de 45 € a partir de 2027."},
 "candidate":{"ref":"AC-CUO-002-01@1","type":"criterion","title":"Prorrateo del primer año","text":"Dado un socio que se da de alta el 15 de marzo de 2027, …"},
 "expected":"update","note":"El criterio sigue siendo válido, pero debe usar 45 €: el primer año serían 33,75 €."}
```

```json
{"id":"I017","partition":"dev",
 "idea":{"text":"Reutilizar los números de socio de las bajas para que la numeración no crezca tanto."},
 "node":{"ref":"DEC-SOC-005@1","type":"decision","title":"Número de socio","text":"Cada socio recibe un número correlativo al darse de alta. …"},
 "expected":"conflicts","note":"La decisión prohíbe reutilizar los números."}
```

| Campo | Contenido |
|---|---|
| `id` | `V001…` en veredictos e `I001…` en ideas. Único y estable. |
| `partition` | `dev` o `test`. |
| `change`, `candidate`, `node` | `ref` (`CODE@version`), `type`, `title` y `text`. El prefijo del código fija el tipo: `DEC` decisión, `FDR`, `ADR`, `AC` criterio, `IDEA` idea y `FUE` fuente. El cambio solo puede ser decisión, FDR, ADR o criterio. |
| `idea` | `text` de la propuesta. |
| `expected` | Veredicto o hallazgo correcto. |
| `note` | Por qué es ese y no otro. |
| `labels` | Opcional. Marca los casos difíciles para medirlos aparte: `injection`, `shared_words`, `implicit_supersession`, `negation` y `synonyms`. |

Cada `ref@version` tiene el mismo tipo, título y texto en todos los casos de los dos ficheros. Los esquemas Zod `verdictCaseSchema` y `ideaCaseSchema` validan el formato, y `packages/domain/test/classifier-evals.test.ts` comprueba el resto de reglas de este documento.
...
1. Se clasifica cada par con el clasificador que se evalúa (`nombre@versión`), sin pasarle la `note` ni las `labels`.
2. `evaluateClassification({ classes: VERDICTS, cases })` recibe `{ expected, actual, confidence }` por caso y devuelve la exactitud, la precisión, la cobertura y el F1 por clase, los macro-promedios, la matriz de confusión y la curva cobertura–precisión por umbral.
...
En la matriz hay que mirar sobre todo `matrix.keep.invalidate`, el error caro de invalidar algo que debía mantenerse. También conviene mirar los resultados por etiqueta, filtrando los casos antes de evaluarlos.
...
- Un caso no se reetiqueta sin dejar constancia en su `note`. Si cambia la semántica de las clases, se crea `v2` en lugar de editar `v1`.
...
- **Las etiquetas son de una sola persona.** Algunos casos frontera, sobre todo `keep` frente a `relate`, `update` frente a `invalidate` o `add` y `conflicts` frente a `inconsistent`, admiten discusión. La `note` explica el criterio aplicado. Una segunda persona debería revisar las etiquetas y medir el acuerdo.
