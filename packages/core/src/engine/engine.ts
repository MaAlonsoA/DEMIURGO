  ...
  await inTransaction(s, async (execute, trx) => {
    ...
    if (exploration?.state === 'active') {
      try {
        ...
        // The engine was checked when the message was posted: if it went away since, the answer is
        // not requested (the person asks again) instead of retrying the workflow forever.
...
async function respondWorkflow(
  ...
  await DBOS.runStep(() => requestResponse(workflow, projectId, explorationId, questionId, agent ?? null), {
    name: 'request',
    ...RETRIES,
  });
}
